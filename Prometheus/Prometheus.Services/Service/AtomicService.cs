...
namespace Prometheus.Services.Service
{
    public class AtomicService
    {
        ...
        public string GetCheckFlagCondition(List<string> variables)
        {
            ...
            var condition = string.Join("||", variables.Select(x => string.Format(variableCondition, x)));
        ...
        {
            var checkAndMarkCondition = $"if(!CAS({snapshot.Variable}, {snapshot.Snapshot}, FLAG({snapshot.Snapshot}, {regionCode})) {{ HELP HERE; continue; }}";
